...
var ft_areaadd = currentForm = new ew.Form("ft_areaadd", "add");
...
ft_areaadd.validateRequired = <?php echo json_encode(CLIENT_VALIDATE) ?>;
...
<?php if ($t_area_add->CheckToken) { ?>
<input type="hidden" name="<?php echo TOKEN_NAME ?>" value="<?php echo $t_area_add->Token ?>">
<?php } ?>
...
<?php if ($t_area->Area->Visible) { // Area ?>
	<div id="r_Area" class="form-group row">
		<label id="elh_t_area_Area" for="x_Area" class="<?php echo $t_area_add->LeftColumnClass ?>"><?php echo $t_area->Area->caption() ?><?php echo ($t_area->Area->Required) ? $Language->Phrase("FieldRequiredIndicator") : "" ?></label>
		<div class="<?php echo $t_area_add->RightColumnClass ?>"><div<?php echo $t_area->Area->cellAttributes() ?>>
