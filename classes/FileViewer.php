...
class FileViewer
{
	...
	{
		global $Conn, $UserProfile, $Security;
		...
		if ($token <> "") {
			...
			// Decrypt if encrypt file path enabled
			if ($validRequest && ENCRYPT_FILE_PATH) {
				$table = Decrypt($table, $key);
				$field = Decrypt($field, $key);
				$recordkey = Decrypt($recordkey, $key);
			}
