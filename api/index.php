...
class Api
{
	...
	{
		if ($token !== NULL) { // Get import progress from file token
			$path = IncludeTrailingDelimiter(UploadPath(TRUE) . UPLOAD_TEMP_FOLDER_PREFIX . $token, TRUE);
			$ar = @json_decode(file_get_contents($path . $token . ".txt"), TRUE);
			if (is_array($ar)) {
				WriteJson($ar);
				$res = TRUE;
			}
